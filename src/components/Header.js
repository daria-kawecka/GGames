...
const Header = ({ text, button }) => {
  ...
  useEffect(() => {
    ...
    gsap.from(".button", { x: 800, scale: 0.5, duration: 1.5 });
    ...
    tl.addLabel("move", "0")
      ...
      .from(panel1, 0.8, { y: "+=15", yoyo: true, repeat: -1 }, "move")
      .from(panel2, 0.8, { y: "-=15", yoyo: true, repeat: -1 }, "move")
      .from(
        smoke,
        1,
        {
  ...
  return (
    <header className="header">
      ...
      <div className="header__text">
        {text ? text : Header.defaultProps.text}
      </div>
      {button ? button : Header.defaultProps.button}
